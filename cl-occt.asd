(asdf:defsystem :cl-occt
  :description "Common Lisp bindings to OCCT CAD kernel"
  :author "Gal Buki"
  :license "MIT"
  :depends-on (:cffi :trivial-garbage :alexandria)
  :serial t
  :components
  ((:module "src"
    :components
    ((:file "package")
     (:module "ffi"
      :components
      ((:file "loader")
       (:file "bindings-common")
       (:file "bindings-primitives")
       (:file "bindings-booleans")
       (:file "bindings-transforms")
       (:file "bindings-geom2d")
       (:file "bindings-topology")
       (:file "bindings-io")
       (:file "bindings-xde")
       (:file "bindings-visual")
       (:file "bindings-ais-types")
       (:file "bindings-lighting")
       (:file "bindings-selection")
       (:file "bindings-geom3d")
       (:file "bindings-helix")
       (:file "bindings-text")
       (:file "bindings-dimensions")
       (:file "bindings-operations")
       (:file "bindings-features")
       (:file "bindings-shape-fix")
       (:file "bindings-mass-props")
       (:file "bindings-mesh")
       (:file "bindings-hlr-conversion")
       (:file "bindings-image-texture")
       (:file "bindings-animation")
       (:file "bindings-graphic3d")
       (:file "bindings-prs3d")
       (:file "bindings-math-inttools")
       (:file "bindings-xcaf")))
      (:module "core"
        :components
        ((:file "shape")
         (:file "errors")
         (:file "primitives")
         (:file "text")
          (:file "geom2d")
          (:file "curves")
          (:file "surfaces")
          (:file "geom-algorithms")
          (:file "helix")
           (:file "faces")
           (:file "booleans")
            (:file "compounds")
            (:file "transforms")
            (:file "assembly")
             (:file "io")
             (:file "xcaf-doc")
             (:file "mass-properties")
            (:file "shape-analysis")
             (:file "topology")
             (:file "mesh")
             (:file "fillet")
             (:file "chamfer")
             (:file "blend")
             (:file "sweep")
             (:file "loft")
              (:file "face-filling")
              (:file "shell")
              (:file "offset")
              (:file "draft")
               (:file "shape-fix")
               (:file "shape-rebuild")
               (:file "shape-process")
                (:file "hole-prism-revol")
                (:file "pipe-feature")
                (:file "local-ops")
                (:file "sewing")
                (:file "defeaturing")
                (:file "shape-check")
                (:file "hlr")
                (:file "shape-conversion")
              (:file "viewer")
            (:file "viewer-colors")
            (:file "viewer-camera")
            (:file "viewer-object-props")
            (:file "viewer-lighting")
            (:file "viewer-grid")
            (:file "viewer-background")
            (:file "viewer-rendering")
            (:file "viewer-text-labels")
            (:file "viewer-defaults")
            (:file "viewer-drawer")
              (:file "viewer-dimensions")
              (:file "viewer-ais-types")
              (:file "animation")
               (:file "texture")
               (:file "materials")
               (:file "selection")
               (:file "graphic3d-clip-plane")
               (:file "graphic3d-shader-program")
                (:file "graphic3d-aspects")
                (:file "graphic3d-structure")
                (:file "graphic3d-group")
                (:file "graphic3d-rendering-params")
                (:file "prs3d-tools")
                (:file "math-optimization")
                (:file "inttools")))))))

(asdf:defsystem :cl-occt/tests
  :description "Tests for cl-occt"
  :depends-on (:cl-occt)
  :components
  ((:module "t"
    :components
    ((:file "test-helpers")
     (:file "primitives-tests")
     (:file "geom2d-tests")
     (:file "curves-tests")
     (:file "gc-tests")
     (:file "faces-tests")
     (:file "booleans-tests")
     (:file "transforms-tests")
     (:file "io-tests")
     (:file "compounds-tests")
     (:file "assembly-tests")
     (:file "viewer-tests")
     (:file "trihedron-camera-tests")
     (:file "selection-tests")
     (:file "lighting-tests")
     (:file "grid-background-rendering-tests")
     (:file "text-labels-dimensions-tests")
     (:file "drawer-tests")
     (:file "colors-tests")
     (:file "font-text-tests")
     (:file "mass-properties-tests")
     (:file "shape-analysis-topology-tests")
     (:file "operations-tests")
     (:file "features-offset-tests")
     (:file "shape-healing-tests")
     (:file "ais-types-tests")
     (:file "mesh-tests")
     (:file "xcaf-tests")
     (:file "graphic3d-tests")
     (:file "animation-structure-group-tests")
     (:file "prs3d-math-inttools-tests")
     (:file "check-hlr-conversion-tests")
     (:file "test-runner"))))
  :perform (test-op (o c)
             (let ((*package* (find-package :cl-occt)))
               (asdf:load-system :cl-occt/tests)
               (multiple-value-bind (pass fail)
                   (funcall (find-symbol "RUN-CORE-TESTS" :cl-occt))
                  (declare (ignore pass))
                  (if (plusp fail) (uiop:quit 1))))))
